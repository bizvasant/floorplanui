...
class quadrants_area(quadrants):
    ...
    def quad_area_info(image,filename):
        ...
        from ocr import ocr
        print(filename)
        df1 = ocr.ocr_info_quard_distro(filename)
        
        # df1 = pd.read_csv("data_img_1.csv")

        # df1=df1[['Features','Area in sq. ft.']].copy()

        # df1.Features=df1.Features.str.lower().replace(['living room-1','living-1','dinning-1','dinning room-1'],'Living/Dinning room-1')\
        #                             .replace(['kitchen-1','dry bal-1','dry-1'],'Kitchen/Dry Bal-1')
        # df1.Features=df1.Features.str.title()
        # df1=df1.groupby('Features').sum('Area in sq. ft.').reset_index()



        print('df1_2_',df1.shape)
        df1 = df1.sort_values(['Area in sq. ft.'],ascending= False)
        df1.reset_index(inplace = True, drop = True)      
        print('df1_',df1.shape)
        ...
        #rooms = rooms[0:len(df1)]

        print('rooms_',rooms.shape)

        features_dict = dict(zip(rooms['Color'], df1['Features']))
        area_dict_1 = dict(zip(rooms['Color'], df1['Area in sq. ft.']))
